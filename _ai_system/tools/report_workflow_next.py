...
def prompt_for(action: str, project_name: str, next_chapter: str) -> str:
    ...
    prompts = {
        ...
        "run_chapter_quality_review": f"{project_name}의 reports/chapter_quality/chapter_quality.json을 확인하고 report_reviewer 관점으로 needs_attention 장을 검수하세요. 각 장이 PRD·TOC·workpack의 질문, 독자 의사결정, 근거, 반론, 잔존 리스크, 시각자료 요구를 실제로 답하는지 판단한 뒤, 필요한 장은 chapter_writer 방식으로 reports/chapters/chNN.html을 수정하고 다시 chapter quality hook을 실행하세요. 이 단계에서는 전체 AGENTS 재독해 대신 compose_report_context.py --stage review --write-packet 또는 해당 장의 --stage chapter --write-packet 결과만 읽으세요.",
        ...
        "run_review_candidate_gates": f"{task_prefix}{project_name}의 review-candidate 및 handoff 검증을 실행해 주세요. workspace validation과 report content validation을 분리해서 보고하고, 실패하면 다음 생산 작업으로 번역해 주세요.",
    }
