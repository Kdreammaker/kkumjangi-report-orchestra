...
1. Report PRD.
   ...
   - Confirm `authoring_structure_profile`, `default_paragraph_mode`, and prose-preferred slots when the artifact could be proposal, review opinion, meeting minutes, education material, manual, public release, or another non-generic document type.
   - Confirm `list_style_preset` when nested numbered or bulleted hierarchy matters. Use `_ai_system/document_presets/LIST_STYLE_PRESETS.md` and preserve source list wording/order unless restructuring is explicitly allowed.
   - Confirm `target_document_format` when DOCX, HWPX-compatible HTML, PDF, or mixed delivery affects fonts, list markers, layout, or verification.
...
2. Report design file, normally `reports/report_design.md`.
   ...
   - The PRD also records `document_type_preset`, `authoring_structure_profile`, `default_paragraph_mode`, `list_style_preset` when relevant, `target_document_format`, `output_language`, `language_variant`, `citation_display_language`, `caption_label_profile`, `style_profile`, `target_reader_tone`, and `protected_spans_policy`.
   - The design file decides A4 margins, target-format typography, palette, cover preset, logo priority, table/chart style, list-marker fallback, and confidentiality warning placement.
   - The design file should record inline-first DOCX/Google Docs/HWPX compatibility decisions. Core visual styling for headings, tables, captions, figures, callouts, cover badges, cover metadata, and approval blocks should be available as literal inline `style=""` values in authored fragments or generated components, with CSS kept as preview/print/fallback support.
...
|---|---|
| Scope | `report_prd/*.md` |
| Document type, authoring structure, list style, language, and style profile | `report_prd/*.md`, `_ai_system/document_presets/INDEX.json`, `_ai_system/document_presets/LIST_STYLE_PRESETS.md`, selected `language_guidance.md`, and `_ai_system/style_profiles/INDEX.json` |
| Report-specific design and target-format visual contract | `reports/report_design.md` |
