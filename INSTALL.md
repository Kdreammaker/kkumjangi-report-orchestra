...
설치 완료 보고에는 다음을 포함하세요.

- 설치 위치
- `00_사용자_작업공간/` 생성 또는 기존 유지 여부
- Python과 필수 패키지 상태
- Docling/DuckDB 로컬 smoke test 상태
- Apache ECharts/Pretendard 로컬 asset 설치 상태
- 로컬 검증기 실행 결과
- 인프라 검증은 통과했지만 보고서 내용 검증은 아직 아니라는 설명
- 설치된 시스템 버전과 배포일
...
2. 설치가 끝났다면 START_HERE.html을 먼저 확인해 주세요.
   이 문서에는 새 프로젝트 시작, 보고서 작성, 검수, 고도화 요청 문구가 정리되어 있습니다.

3. 설치 직후에는 같은 환경 점검 프롬프트를 다시 보낼 필요가 없습니다.

4. 이 시스템에 특히 적합한 문서 유형은 시장조사, 기업 비교분석, 전략 보고서, 정책/규제 검토, 서비스 기획/PRD, 기술 도입 검토입니다.

5. 처음 테스트하기 좋은 샘플 주제는 다음과 같습니다.
   - 국내 커피 프랜차이즈 시장 변화와 신규 진입 전략
   - 주요 협업툴 3종 비교와 도입 검토
   - 중소 제조기업의 AI 도입 기회와 리스크

6. 바로 시작하려면 AI에게 이렇게 요청하세요.
   새 프로젝트를 시작해 주세요.
   주제: [작성하고 싶은 보고서 주제]
   한 줄 목적: [왜 작성하는지]
```
