...
- Recheck/update requests may compare local `VERSION.json`/HEAD with `origin/main`, but do not update blindly. Pull only with private origin, no system-core local changes, and a fast-forward path. If the workspace is a ZIP install, public origin, dirty, conflicted, or ambiguous, report status and do not run destructive repair or overwrite commands.
