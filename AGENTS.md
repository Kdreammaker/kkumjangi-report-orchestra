...
- Recheck/update requests may compare local `VERSION.json`/HEAD with `origin/main`, but do not update blindly. Pull only when `origin` matches the installed release channel/repository, there are no system-core local changes, and the update can fast-forward. If the workspace is a ZIP install, has an unexpected origin, is dirty, conflicted, or ambiguous, report status and do not run destructive repair or overwrite commands.
