...
This project installs or uses the following third-party components for local-only document processing, indexing, rendering, and report production.
...
Default operation is local-only. These dependencies do not upload user files by themselves in this system. External OCR, external image interpretation, cloud upload, or connector handoff requires separate user approval.
